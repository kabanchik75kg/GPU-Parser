...
def get_all_pages(base_url):
    """Получает список всех страниц с видеокартами"""
    all_pages = [base_url]
    try:
        ...
        # Ищем пагинацию
        pagination = soup.find(class_='rating_pages_links')
        if pagination:
            last_page = pagination.find_all('a')[-1].get('href')
            if last_page:
                total_pages = int(last_page.split('=')[-1])
                for page in range(2, total_pages + 1):
                    all_pages.append(f"{base_url}?page={page}&pg={page}")
    except Exception as e:
        print(f"Ошибка при получении списка страниц: {e}")
    return all_pages
